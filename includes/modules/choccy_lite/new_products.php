...
if ($num_products_count > 0) {
  ...
  while (!$new_products->EOF) {
    $products_price = zen_get_products_display_price_choccy_lite($new_products->fields['products_id']);
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="centerBoxContentsNew float-left ' . $additional_class . '"' . ' ' . 'style="width:' . $col_width . '%;"',
    'text' => (($new_products->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<div class="columnar-product-info"><div class="columnar-product-img"><a href="' . zen_href_link(zen_get_info_page($new_products->fields['products_id']), 'cPath=' . $productsInCategory[$new_products->fields['products_id']] . '&products_id=' . $new_products->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $new_products->fields['products_image'], $new_products->fields['products_name'], IMAGE_PRODUCT_NEW_WIDTH, IMAGE_PRODUCT_NEW_HEIGHT) . '</a></div>') . '<div class="product-box-name"><a href="' . zen_href_link(zen_get_info_page($new_products->fields['products_id']), 'cPath=' . $productsInCategory[$new_products->fields['products_id']] . '&products_id=' . $new_products->fields['products_id']) . '">' . substr($new_products->fields['products_name'], 0, 40) . (strlen($new_products->fields['products_name']) > 40 ? "..." : "") . '</a></div>' . $products_price . '</div><a class="detail-link" href="' . zen_href_link(zen_get_info_page($new_products->fields['products_id']), 'cPath=' . $productsInCategory[$new_products->fields['products_id']] . '&products_id=' . $new_products->fields['products_id']) . '">'. DETAIL_LINK .'</a>');
